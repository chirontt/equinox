...
public class FreeBSDPasswordProvider extends PasswordProvider implements IValidatingPasswordProvider {
	...
	public PBEKeySpec getPassword(IPreferencesContainer container, int passwordType) {
		...
		if (!newPassword && !passwordChange) {
			try {
				...
				AuthPlugin.getDefault().logError(FreeBSDPasswordProviderMessages.getMasterPasswordError, e);
				return null;
			}
		}
		...
		AuthPlugin.getDefault().logMessage(FreeBSDPasswordProviderMessages.newMasterPasswordGenerated);

		try {
			saveMasterPassword(newPasswordString);
			return new PBEKeySpec(newPasswordString.toCharArray());
		} catch (SecurityException e) {
			AuthPlugin.getDefault().logError(FreeBSDPasswordProviderMessages.saveMasterPasswordError, e);
			return null;
		}
	}
